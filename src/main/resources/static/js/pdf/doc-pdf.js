...
function doConvert() {
    ...
    if (flag=="true") {
        ...
        $(convertForm).ajaxSubmit({
            dataType: "json",
            beforeSend: function () {
                $(convert).attr(DISABLED, true).val("转化中...");
            },
            success: function (result) {
                if(backDetectResult(result)){
                    ...
                    addSuccessMsg(msgContainer, '<p><a onclick="download(this)" data-date="'+date+'" data-name="'+name+'" data-downloadName="'+downloadName+'">' + name +'</a></p>')
                }
            },
        ...
        complete: function () {
                $(convert).removeAttr(DISABLED).val("转化");
            }
        });
    }
